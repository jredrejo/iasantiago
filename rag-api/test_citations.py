...
Prueba que las citas en el contexto RAG usen la página correcta.
...
def test_citations(topic: str, query: str):
    ...
    logger.info("\n" + "=" * 80)
    logger.info("🔍 ANÁLISIS DE CITAS EN EL CONTEXTO")
    logger.info("=" * 80)
    ...
    logger.info(f"\n📊 Citas encontradas: {len(citations)}")

    if not citations:
        logger.error("❌ ¡NO HAY CITAS! Algo va mal")
        logger.info(f"\nContext preview:\n{context_text[:500]}")
        return
    ...
    for i, (citation_file, citation_page) in enumerate(citations[:5], 1):
        ...
        if matching_chunk:
            ...
            logger.info(f"      Citation dice: p.{citation_page}")
            logger.info(f"      Página actual:   p.{actual_page}")

            if not match:
                all_match = False
                logger.error(
                    f"         ⚠️  ¡DISCREPANCIA! {actual_page} ≠ {citation_page_int}"
                )
        else:
    ...
    logger.info("\n" + "=" * 80)
    if all_match:
        logger.info("✅ ¡TODAS LAS CITAS SON CORRECTAS!")
    else:
        logger.error("❌ ¡HAY DISCREPANCIAS EN LAS CITAS!")
        logger.error("   Esto significa que attach_citations_explicit() está usando")
        logger.error(
            "   el valor incorrecto. Revisa que esté usando 'page', no 'chunk_id'"
        )
    logger.info("=" * 80 + "\n")

    # Mostrar ejemplo de cita en contexto
    logger.info("📖 EJEMPLO DE CITA EN CONTEXTO:\n")
...
def main():
    if len(sys.argv) < 3:
        print("Uso: python test_citations.py <TEMA> <QUERY>")
